...
# The first two wildcars in this are the API and the API's version. The rest
...
class _Registry(object):
  ...
  def Register(self, api, urls_only):
...
def RegisterAPI(api, urls_only=False):
  ...
  Args:
    ...
    urls_only: bool, True if this API should only be used to interpret URLs,
        and not to interpret collectionpaths.
  """
  _REGISTRY.Register(api, urls_only)
...
def ParseResource(line, kwargs=None, collection=None, resolve=True):
  """Parse a Cloud resource from a command line.

  Args:
    line: str, The argument provided on the command line.
    kwargs: {str:str}, The keyword argument context.
  ...
  if line and line.startswith('https://'):
    return _REGISTRY.ParseURL(line, collection)
    ...
    return _REGISTRY.ParseCollectionPath(
        collection, line, kwargs, resolve)
