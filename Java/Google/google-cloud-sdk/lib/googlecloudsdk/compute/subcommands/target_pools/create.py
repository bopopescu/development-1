...
class CreateTargetPool(base_classes.BaseAsyncMutator):
  ...
  def Args(parser):
    ...
    health_checks = parser.add_argument(
        '--health-checks',
        default=[],
        help=('Specifies HttpHealthChecks to determine the health of instances '
              'in the pool.'),
        metavar='HEALTH_CHECK',
        nargs='+')
    health_checks.detailed_help = """\
        Specifies HTTP health check resources to use to determine the health
    ...
    instances = parser.add_argument(
        '--instances',
        default=[],
        help=('Specifies a list of instances included in this target pool.'),
        metavar='INSTANCE',
        nargs='+')
    instances.detailed_help = """\
        Specifies a list of instances that will receive traffic
  ...
  def CreateRequests(self, args):
    ...
    health_checks = [self.context['path-handler'].Normalize(
        'httpHealthChecks', hc) for hc in args.health_checks]
    ...
    request = messages.ComputeTargetPoolsInsertRequest(
        targetPool=messages.TargetPool(
            backupPool=self.context['path-handler'].Normalize(
...
CreateTargetPool.detailed_help = {
