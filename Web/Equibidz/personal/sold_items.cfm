...
<cfset logOperator = "AND">
       ...
       <cfset field = "i.title">
        ...
        <cfloop index="l" list="#search_text#" delimiters=" ">
         ...
         <cfset sql_code = "WHERE #sql_code# AND i.user_id = #session.user_id# AND i.itemnum = b.itemnum AND b.winner = 1">
...
SELECT i.status, i.title,i.date_start, i.date_end, i.buynow_price, b.quantity,b.bid,b.itemnum,b.user_id
FROM   items i, bids b 
...
AND (i.status = 0 OR i.date_end < #TIMENOW#)
...
<cfelse>
AND status = 0
AND date_end > #dateadd("d",-90,timenow)#
...
<TABLE width="100%" border=0>
  ...
  <TR>
    <TD width="100%"><font size=4>Sold Items</font></TD></TR></TABLE>



<TABLE height=48 width="100%" border=0>
  <form action="sold_items.cfm" method="get">
  <TR>
...
<TABLE cellPadding=6 width="100%" bgcolor=F2FFCA border=0>
  ...
  <TR>
	...
	<tr><td colspan=8> <cfif getItems.recordcount gt countgroup>
	    <br>  Click here for more items: 
                <cfmodule template="../functions/pagelinks.cfm"
                  ...
                  link="#VAROOT#/personal/sold_items.cfm?user_id=#session.user_id#&status=0&limit_results=#limit_results#&search_text=#urlencodedformat(search_text)#">
	   </cfif></td></tr>
	...
	<td><FONT size=2><b><u>Winning Bid</u><br>BuyNow Price</b></td>
...
<td><FONT size="1"> #dateformat(getItems.date_end,"mm/dd/yyyy")# #timeformat(getItems.date_end,"hh:mm tt")#</FONT></td>                                                                                                                                       
        <td><FONT size="1"> <u>#dollarformat(getItems.bid)#</u><!--- <br>#dollarformat(getItems.buynow_price)# ---></FONT></td>   
