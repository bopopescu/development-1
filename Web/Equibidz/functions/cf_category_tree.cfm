...
<!---
     ...
     Copyright Beyond Solutions, Inc. 1999
     February 26, 1999
...
<cfif #isDefined ("Attributes.any")# is "1">
 <cfset #any# = 1>
<cfelse>
 <cfset #any# = 0>
</cfif>
...
<cfif #level# is "0">
    <cfquery username="#db_username#" password="#db_password#" name="get_top_info" dataSource="#Attributes.DATASOURCE#">
     SELECT child_count
       FROM categories
      WHERE category = 0
      ORDER BY name
...
<cfquery username="#db_username#" password="#db_password#" name="get_parents" dataSource="#Attributes.DATASOURCE#">
 SELECT category,
        name,
        parent,
        child_count
...
<cfswitch expression=#type#>
 ...
 <cfcase value="SELECT">
  ...
  <cfif #any# EQ 1>
    <cfoutput><option value="0" <cfif selectname GT 0>selected</cfif>>---Any---</option></cfoutput>     
  </cfif>
