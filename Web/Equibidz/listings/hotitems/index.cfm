...
<cfquery username="#db_username#" password="#db_password#" name="getall" datasource="#DATASOURCE#">

SELECT     COUNT(itemnum) AS total, itemnum 
FROM         bids
GROUP BY itemnum
HAVING      (COUNT(itemnum) >= #hotauction.bids#)
order by newid()
</cfquery>

<cfset list="">
<cfloop query="getall">
<CFSET list = ListAppend(List,itemnum)>
</cfloop>

<cfquery name="get_Items"  datasource="#datasource#" maxrows="#evaluate(int(group)*int(get_PageGroup.itemsperpage))#" >
      ...
      <table border=0 cellspacing=0 cellpadding=0 noshade width=640>
        <tr>
		  ...
		  <cfmodule template="../../functions/sortorderlinks.cfm"
		    sortby="#sortby#"
		    action="#VAROOT#/listings/hot/index.cfm">
            </font>
          </td>
        </tr>
      </table>
	...
	<cfcatch>
	  ...
	  <table border=0 cellspacing=0 cellpadding=0 noshade width=640>
        <tr>
		  ...
		  <hr size="1" noshade width="100%" color=<cfoutput>#heading_color#</cfoutput>>
            <b>last updated:</b> <cfoutput>#DateFormat(TIMENOW, "mm/dd/yyyy")#, #TimeFormat(TIMENOW, "HH:mm:ss")# #TIME_ZONE#</cfoutput>
            <br>
            <font size=2>
              <br>
