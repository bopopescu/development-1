...
<cfparam name="cyber_cash" default="no"> 
...
<cfquery username="#db_username#" password="#db_password#" name="SellersCC" datasource="#datasource#">
	...
	and i.itemnum = #int(attributes.itemnum)#
</cfquery>
...
<cfif cyber_cash neq "no" or link_point neq "no">
	...
	<cfif 	len(trim(sellerscc.cc_name)) GT 0 AND
		len(trim(cc_number)) GT 0 AND
		len(trim(sellerscc.cc_expiration)) GT 0 >
		<cfif 	cc_number eq 0 OR
		    len(trim(sellerscc.cc_name)) lt 1 OR
			len(trim(cc_number)) lt 14 OR
			len(trim(sellerscc.cc_expiration)) lt 1 OR
		...
		<cfelse>
		    	...
		    	<cfset cc_cpi_card_exp = trim(sellerscc.cc_expiration)>
	...
	<!--- End CybeCash --->
         ...
         <cfset TempValue = querySetCell(ItemContext,"description","Auction Fees for: #SellersCC.title#")>
...
<cfset CSIOrderFieldError="Transaction not intialized">
		...
		<cfelseif isDefined('CSIOrderFieldApproved')>
			<cfif #CSIOrderFieldApproved# eq "approved">
				...
				<cfset caller.billing_reference = "Order Num: " & "#attributes.itemnum#" &  " Transaction Num: " & "#CSIOrderFieldOrdernum#" & " Transaction Time: " & "#CSIOrderFieldTime#" & " Approved Status: " & "#CSIOrderFieldApproved#" & " Order Error: " & "No Errors">
...
<cfset cc_cpi_card_exp ="#LP_ORDERCARDEXPMONTH#/#LP_ORDERCARDEXPYEAR#">
				...
				<cfset card_number = cc_cpi_card_number>
