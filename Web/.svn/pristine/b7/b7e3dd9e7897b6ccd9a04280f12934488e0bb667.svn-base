<!---
/admin/reinstate_menu.cfm

  This Menu to accepts data to reinstate invoices and payments  on the site. After this process runs, the processed invoices and payments are displayed on the site.
...
<cfinclude template="../includes/session_include.cfm">
 ...
 <cfif isDefined ("reinstate_flag")>
  <cfset #reinstate_flag# = #trim (reinstate_flag)#>
 <cfelse>
  <cfset #reinstate_flag# = 0>
 </cfif>
...
<!---  1/01 Store info in session variables and branch to another form to process reinstatement --->

  <cfif IsDefined("submit3")>
    <cfif IsDefined("form.selected_item")>
      <cfset session.reinstate_flag = #reinstate_flag#>
      <cfset session.selected_item = #form.selected_item#>
	  <cflocation url="reinstate_process.cfm">
	</cfif>
 </cfif>
           ...
           <tr>
            <td colspan=2>
             <font face="helvetica" size=3 color=000000>
              <b>Use this page to Reinstate Invoices and payments that have been previously <br>archived/removed.
To start, select user and click on Invoices by user. After doing so, <br> click on 'Reinstate' and 'Process' to have Reinstatement  processing initiated. 
			 <br><br>
           ...
           <tr>
            <form name="reinstForm" action="reinstate_menu.cfm" method="post">
              ...
              <table border=0 bordercolor=ffffff cellspacing=0 cellpadding=3 bgcolor=bac1cf>
                <cfif #isDefined ("submit2")#>
                 <cfif #submit2# is "Invoices by User">
                  <tr>
                   <cfoutput><td><font face="helvetica" size=2>&nbsp;<b>Please select archived invoices to reinstate from the list below:</b><br>&nbsp;(#get_user_list.recordcount# records found)</font></td></cfoutput>
                   <!---<td>&nbsp;</td>--->
                  </tr>
                  <tr>
                   <td>
                  ...
                  <tr>
                   <td>
                    <table border=0 cellspacing=0 cellpadding=2>
                     <tr>
                      <td><font face="helvetica" size=2><b>Reinstate?:</b></font></td>
                      <td>&nbsp;</td>
                      <td><input name="reinstate_flag" type="checkbox" value="1"<cfif #reinstate_flag# is "1"> checked</cfif>>
	                  </td>
                      <td>&nbsp;</td>
                      <td colspan=1>
