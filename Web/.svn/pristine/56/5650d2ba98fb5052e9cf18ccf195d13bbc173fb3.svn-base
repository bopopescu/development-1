...
<cfif #chk_duplicate.RecordCount# EQ 0>
   <cfquery username="#db_username#" password="#db_password#" name="insert_item" datasource="#DATASOURCE#">
        ...
        VALUES (1,
             '#session.category#',
             #session.category1#,
             '#session.auction_type#',
             #session.auction_mode#,
             '#session.list_type#',
             #itemnum#,
             #session.user_id#,
             '#session.name#',
   ...
   <cfmail to = "#get_Seller.email#"
         ...
         Your item, "#session.name#", has been added to our item database under the listing "#get_CatName.name#".
         
         It will be up for auction starting #dateFormat(session.date_start)#, and will run for #session.duration# day<cfif #session.duration# GT 1>s</cfif>, 
         closing on #dateFormat(session.date_end)#. For your reference, the item number #itemnum# automatically assigned to your item.  
         It would be a good idea to write this number down for future reference to your item.
         
         You may view this auction in progress at http://#SITE_ADDRESS##VAROOT#/listings/details/index.cfm?itemnum=#itemnum#&curr_cat=#curr_cat#&curr_lvl=0&from_search=0.
	...
	<div align="center">
      ...
      <tr><td>&nbsp;</td></tr>
  	  <cfif #new_item# is "TRUE">
         ...
         <tr><td>&nbsp;</td></tr>
         <tr><td>
            Your item, "#session.name#", has been added to our item database under the listing "#get_CatName.name#".<br><br>
            It will be up for auction starting #dateFormat(session.date_start)#, and will run for #session.duration# day<cfif #session.duration# GT 1>s</cfif>, closing on #dateFormat(session.date_end)#.<br><br>
