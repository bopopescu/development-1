...
@property (nonatomic, strong) DHAPBeaconCat *cat;
...
- (void)viewDidLoad
{
    ...
    self.uuid = [[DHAPBeaconCatManager sharedInstance] uuid];
        ...
        if (beacon.proximity != _currentProximity) {
            ...
            switch (_currentProximity) {
                ...
                case CLProximityNear:
                    ...
                    break;
                case CLProximityFar:
                    [self logProximity:@"Far" forBeacon:beacon];
                    break;
            }
        }
    }
...
- (void)setupRanging
{
    CLBeaconRegion *region = [DHAPBeaconCat catsRegion];
    if (region) {
...
- (void)setupMonitoring
{
    ...
    for (DHAPBeaconCat *cat in self.cats) {
...
- (void)setCatForMajor:(NSNumber *)major andMinor:(NSNumber *)minor
{
    ...
    NSUInteger index = [self.cats indexOfObjectPassingTest:^(id obj, NSUInteger idx, BOOL *stop) {
        DHAPBeaconCat *cat = (DHAPBeaconCat *)obj;
